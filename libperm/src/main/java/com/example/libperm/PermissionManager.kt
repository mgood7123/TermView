...
import com.example.liblayout.views.view.Button
import com.example.libutils.core.deleteRecursive
                            ...
                            button().also {
                                ...
                                it.setOnClickListener {
                                    Thread {
                                        ...
                                        // find apk
//                                        val pkg = "a.termview"
                                        val pkg = "com.gmail.heagoo.apkeditor.pro"
                                        Log.i("pkg", "pkg = $pkg")
                                        ...
                                        if (dir.exists()) {
                                            // if dir already exists, do not decompile the apk
                                            // instead assume last attempt failed and try to rebuild

                                            // rebuild apk
                                            val cmdr = arrayOf("-v", "build", dir.path)
                                            val retr = com.example.apktool.brut.apktool.Main.main(cmdr)
                                            ...
                                            val cmd = arrayOf("-v", "decode", apk.path, "-o", dir.path)
                                            val ret = com.example.apktool.brut.apktool.Main.main(cmd)
