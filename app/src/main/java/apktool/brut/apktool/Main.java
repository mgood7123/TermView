 ...
 */
package apktool.brut.apktool;

import android.support.annotation.NonNull;
import apktool.brut.androlib.*;
    ...
    public static int main(String[] args) throws IOException, InterruptedException, BrutException {
        ...
        for (String opt : commandLine.getArgs()) {
            if (opt.equalsIgnoreCase("d") || opt.equalsIgnoreCase("decode")) {
                returnValue = cmdDecode(commandLine);
                cmdFound = true;
            } else if (opt.equalsIgnoreCase("b") || opt.equalsIgnoreCase("build")) {
                returnValue = cmdBuild(commandLine);
                cmdFound = true;
        ...
        // if no commands ran, and returnValue is still zero, run the version / usage check.
        if (!cmdFound && returnValue == 0) {
