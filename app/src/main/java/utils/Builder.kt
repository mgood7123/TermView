...
import utils.`class`.extensions.isGreaterThan
...
class Builder(var maxHeight: Int?, var maxWidth: Int?) {
    ...
    fun height(percentage: Int): Builder {
        if (percentage isLessThan 0 || percentage isGreaterThan 100) throw IllegalArgumentException(
    ...
    fun height(percentage: Double): Builder {
        if (percentage isLessThan 0.0 || percentage isGreaterThan 100.0) throw IllegalArgumentException(
            "value must be between 0 and 100"
        )
        if (!threaded) {
            row[row.lastIndex].customHeight = true
    ...
    private fun resize() {
        when {
            row.any { it.customHeight } -> when {
                row.sumByDouble { it.percentage } isGreaterThan 100.0 -> throw ArithmeticException()
                !row.all { it.customHeight } -> {
                    ...
                    if (row.sumByDouble { it.percentage } isGreaterThan 100.0) throw ArithmeticException()
